...
class DECA(nn.Module):
    ...
    def encode(self, images, use_detail=True):
        ...
        if use_detail:
            ...
            # codedict['detail'] = torch.zeros(detailcode.shape).cuda()
    ...
    # @torch.no_grad()
    def decode(self, codedict, rendering=True, iddict=None, vis_lmk=False, return_vis=True, use_detail=True,
        ...
        if return_vis and render_orig and original_image is not None and tform is not None:
            ...
            images = original_image.to(torch.float32)
        else:
        ...
        if return_vis:
            ...
            img_temp = util.tensor2image(ops_detail['images'][0])
            cv2.imwrite("/home/wenchi/zxy/HSD/test_detail.jpg", img_temp)
            visdict = {
                'inputs': images, 
    ...
    def fast_decode(self, codedict, tform=None, original_size = 512):
        ## fast decode only calculate opdict for verts, trans_verts, landmarks2d, landmarks3d, trans_verts
        ...
        points_scale = [self.image_size, self.image_size]
        h, w = original_size, original_size
        # import ipdb; ipdb.set_trace()
        trans_verts = transform_points(trans_verts, tform, points_scale, [h, w])
        landmarks2d = transform_points(landmarks2d, tform, points_scale, [h, w])
        landmarks3d = transform_points(landmarks3d, tform, points_scale, [h, w])
    ...
    def render_for_hsd(self, codedict, original_image=None, tform=None):
        ...
        zero_detail_code = torch.zeros((codedict['pose'].shape[0], 128)).cuda()
        ...
        ## 0~1 , B*3*512*512, RGB, tensor, cuda
