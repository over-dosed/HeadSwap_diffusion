...
class DDPM(pl.LightningModule):
    ...
    def get_input(self, batch, k):
        if k == "inpaint":
            x = batch['target']
            mask = batch['mask']
            inpaint = batch['background']
            reference_global = batch['source_global']
            reference_id = batch['source_id']
        else:
            x = batch[k]
        if len(x.shape) == 3:
            x = x[..., None]
        ...
        reference_global = reference_global.to(memory_format=torch.contiguous_format).float()
        reference_id = reference_id.to(memory_format=torch.contiguous_format).float()
        return x, inpaint, mask, reference_global, reference_id
    ...
    def self_log_dict(self, dict):
        self.log_dict(dict, prog_bar=False, logger=True, on_step=False, on_epoch=True)
...
class LatentDiffusion(DDPM):
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=True,
        ...
        if bs is not None:
            ...
            inpaint = inpaint[:bs]
            mask = mask[:bs]
            reference_global = reference_global[:bs]
            reference_id = reference_id[:bs]
        x = x.to(self.device)
        encoder_posterior = self.encode_first_stage(x)
        z = self.get_first_stage_encoding(encoder_posterior).detach()
